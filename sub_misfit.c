...
SOLN get_tshift_corr_misfit(
  ...
  for(obs=obs0,sol.wferr=0.,i=0;i<nda;i++,obs++){
    ...
    for(kc=2,f_pt1=rad+NRF,j=0;j<NCP;j++,spt++,kc=NRF,f_pt1=rad) {
        ...
        // MISFIT FUNCTION
        // 2023-07-07 NOTE rec2 includes rew (reward) in main. 
        // Disabled - Want to compare waveform fit across events, but reward
        // does not allow one-to-one since it requires same win len and freq.
        x1 = spt->rec2 + (x2 * y1 * y1) - 2.*(cfg[j] * y1);     // (x-y)^2 = x^2 + y^2 - 2xy
        //fprintf(stderr,"%d\n",spt->npt);
        ...
        x1 = x1/spt->npt;
        sol.error[i][j] = spt->on_off*x1;	// L2 error for this com. on/off switch
        sol.cfg[i][j] = 100*cfg[j]/sqrt(spt->rec2*x2);  // 2023-07-26 x-corr per component 
        sol.wferr += spt->on_off*x1;                    // 2023-07-26 used to calc (1) percent weight per component and (2) repeat-discard operation(=obsolete).
    }
  }
  return(sol);
}
